  ...
  <div>
    ...
    <el-form inline :model="searchParam" size="small" class="search">
      <el-form-item label="搜索：">
        <el-input v-model="searchParam.keyword" placeholder="用户名/昵称/手机号/邮箱"></el-input>
      </el-form-item>
      <el-form-item>
        ...
        rules: {
          add: {
            username: [
              ...
              {pattern: '^[a-zA-Z][a-zA-Z0-9]{3,9}$', message: '用户名必须4 到 10位的数字或字母，字母开头', trigger: 'blur'}
            ],
            nickname: [
    ...
    methods: {
      ...
      deleteUser(id){
        this.$confirm('确定要删除吗?', '警告', {
          ...
          this.$axios.delete(this.GLOBAL.baseurl + '/sys/user/delete/' + id, {
            withCredentials: true
          }).then(res => {
            this.$message.success(res.data.msg);
            this.loadData();
       ...
       */
      resetpwd(id){
        this.$axios.put(this.GLOBAL.baseurl + '/sys/user/resetPwd/' + id, null, {
          withCredentials: true
        }).then(res => {
          this.dialogForm.update = false;
          this.$message.success(res.data.msg);

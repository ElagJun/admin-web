<template>
  <div>
      ...
      <el-col :span="12">
        ...
        <el-form label-width="80px" disabled :model="infoParam" class="dialogStyle">
          ...
          <el-form-item label="权限字符">
            <el-input size="small" v-model="infoParam.permission"/>
          </el-form-item>
    ...
    <el-dialog title="添加" :visible.sync="dialogForm.add" width="30%" @close="closeEditFrom(true)" class="dialogStyle">
      <el-form label-width="80px" ref="addParam" :model="addParam" :rules="rules.add">
        ...
        <el-form-item label="权限字符" prop="permission">
          <el-input size="small" v-model="addParam.permission"/>
        </el-form-item>
    ...
    <!-- 资源修改 -->
    <el-dialog title="修改" :visible.sync="dialogForm.update" width="30%" @close="closeEditFrom(false)" class="dialogStyle">
      <el-form label-width="80px" ref="updateParam" :model="updateParam" :rules="rules.update">
        ...
        <el-form-item label="权限字符" prop="permission">
          <el-input size="small" v-model="updateParam.permission"/>
        </el-form-item>
        <el-form-item label="图标" prop="icon">
    ...
    methods: {
      ...
      visibleEditForm(data) {
        ...
        if(data.id){
          ...
          this.updateParam.permission = data.permission;
       ...
       */
      updateRes(){
        if(this.updateParam.parentId == null){
      ...
      deleteRes(data) {
        this.$confirm('确定要删除吗?', '警告', {
          ...
          this.$axios.delete(this.GLOBAL.baseurl + '/sys/res/delete/' + data.id, {
            withCredentials: true
          }).then(res => {
            this.$message.success(res.data.msg);
            this.loadData();
